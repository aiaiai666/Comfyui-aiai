...
def _comfly_normalize_output_format(output_format):
...
def _comfly_output_extension(output_format):
    fmt = _comfly_normalize_output_format(output_format)
    return "jpg" if fmt == "jpeg" else fmt

def _comfly_decode_base64_image(b64_data):
    if not b64_data:
        return None
...
def _comfly_save_pil_image_url(pil_img, filename_prefix="aiai/gpt_image_2", output_format="png", output_compression=100):
    fmt = _comfly_normalize_output_format(output_format)
    ext = _comfly_output_extension(fmt)
    save_img = pil_img.copy()
    width, height = save_img.size
    if fmt == "jpeg" and save_img.mode != "RGB":
        save_img = save_img.convert("RGB")
    elif fmt in ("png", "webp") and save_img.mode not in ("RGB", "RGBA"):
        save_img = save_img.convert("RGBA" if "A" in save_img.getbands() else "RGB")
    ...
    save_kwargs = {}
    if fmt in ("jpeg", "webp"):
        save_kwargs["quality"] = max(1, min(100, int(output_compression or 100)))
    elif fmt == "png":
        save_kwargs["compress_level"] = 4
    save_img.save(save_path, format=fmt.upper(), **save_kwargs)
...
def _comfly_base64_image_to_tensor_url(b64_data, output_format="png", filename_prefix="aiai/gpt_image_2", output_compression=100):
    pil_img = _comfly_decode_base64_image(b64_data)
    if pil_img is None:
        return None, ""
    image_url = _comfly_save_pil_image_url(
        pil_img,
        filename_prefix=filename_prefix,
        output_format=output_format,
        output_compression=output_compression,
    )
    return pil2tensor(pil_img), image_url

def _comfly_maybe_base64_image_to_tensor_url(value, output_format="png", filename_prefix="aiai/gpt_image_2", output_compression=100):
    value = str(value or "").strip()
    if not value:
        return None, ""
    if value.startswith("data:image"):
        return _comfly_base64_image_to_tensor_url(
            value,
            output_format=output_format,
            filename_prefix=filename_prefix,
            output_compression=output_compression,
        )
    if value.startswith(("http://", "https://", "/view?")):
        return None, ""
    try:
        return _comfly_base64_image_to_tensor_url(
            value,
...
class Comfly_api_set:
    ...
    def set_api_base(self, api_base, apikey="", skip_error=False):
        global baseurl
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "model_version": (["v 8.1","v 8","v 7", "v 6.1", "v 6.0", "v 5.2", "v 5.1", "niji 7", "niji 6", "niji 5", "niji 4"], {"default": "v 8.1"}),
...
class Comfly_mjstyle:
    ...
    def __init__(self):
        dir = os.path.join(os.path.dirname(__file__), "web", "mjstyle")
...
class Comfly_gpt_image_2_official:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "output_compression": ("INT", {"default": 100, "min": 0, "max": 100}),
                "moderation": (["auto", "low"], {"default": "auto"}),
                "async_mode": ("BOOLEAN", {"default": False, "tooltip": "Compatibility only. This node follows /v1/images/edits and returns directly."}),
                "webhook": ("STRING", {"default": "", "tooltip": "Compatibility only. Ignored by this node."}),
                "max_poll_attempts": ("INT", {"default": 300, "min": 10, "max": 1000, "tooltip": "Compatibility only. Ignored by this node."}),
                "poll_interval": ("INT", {"default": 5, "min": 2, "max": 60, "tooltip": "Compatibility only. Ignored by this node."}),
                "max_retries": ("INT", {"default": 5, "min": 1, "max": 10}),
                "initial_timeout": ("INT", {"default": 900, "min": 60, "max": 1200}),
                "seed": ("INT", {"default": 0, "min": 0, "max": 0xffffffffffffffff}),
                "skip_error": ("BOOLEAN", {"default": False, "tooltip": "开启后，节点失败时不报错、按旧行为返回默认空结果；关闭时（默认）失败直接抛出错误。"})
    ...
    def _decode_b64_url_one(self, b64_json, image_url, max_retries, initial_timeout, output_format="png", output_compression=100):
        """One image entry to (tensor, url). Base64 responses are saved to output first."""
        if b64_json:
            return _comfly_base64_image_to_tensor_url(
                b64_json,
                output_format=output_format,
                filename_prefix="aiai/gpt_image_2_official",
                output_compression=output_compression,
            )
        if image_url:
            t, saved_url = _comfly_maybe_base64_image_to_tensor_url(
                image_url,
                output_format=output_format,
                filename_prefix="aiai/gpt_image_2_official",
                output_compression=output_compression,
    ...
    def _extract_image_items(self, payload):
        """Find image result objects in both OpenAI-style and provider task payloads."""
        items = []
        ...
        def walk(value):
            if isinstance(value, dict):
                ...
                if isinstance(value.get("image_url"), str):
                    items.append({"url": value.get("image_url")})
                    return
                data = value.get("data")
                if isinstance(data, (dict, list)):
                    walk(data)
                for key in ("result", "output", "response", "images"):
                    if key in value:
                        walk(value[key])
    ...
    def _extract_task_status(self, payload):
        ...
        for container in containers:
            ...
            if status:
                return str(status), str(container.get("progress", ""))
        return "", ""

    def _items_to_tensors(self, result, max_retries=5, initial_timeout=300, output_format="png", output_compression=100):
        """Parse Images API data[] b64_json or url into tensors and displayable URLs."""
        out = []
        urls = []
        for item in self._extract_image_items(result):
            t, image_url = self._decode_b64_url_one(
                item.get("b64_json", ""),
                item.get("url", ""),
                max_retries,
                initial_timeout,
    ...
    , skip_error=False):
        ...
        def _info_common(mode_line):
            s = f"**AIAI gpt-image-2 (official)** {mode_line}\n"
            s += f"Model: {model}\n"
            s += f"Prompt: {prompt}\n"
            s += f"Quality: {quality}\n"
            if size != "auto":
        ...
        try:
            ...
            mode = "/v1/images/edits (multipart" + (
                ", blank ref" if image is None else ""
            ) + (", mask" if mask is not None else "") + ")"

            if not self._extract_image_items(result):
            ...
            tensors, image_urls = self._items_to_tensors(
                result, max_retries, initial_timeout, output_format, output_compression
            )
            pbar.update_absolute(95)
            ...
            if image_urls_string:
                info += f"Image URL: {image_urls_string}\n"
            if "usage" in result:
                u = result["usage"]
            ...
            return (combined, image_urls_string, info)
    ...
    def _decode_b64_url_one(
        ...
        if b64_json:
            return _comfly_base64_image_to_tensor_url(
                b64_json,
                output_format=output_format,
                filename_prefix="aiai/gpt_image_2_official_ratio",
                output_compression=output_compression,
            )
        if image_url:
            t, saved_url = _comfly_maybe_base64_image_to_tensor_url(
...
class Comfly_gpt_image_2:
    """GPT-Image-2 text-to-image node using /v1/images/generations."""
    ...
    def __init__(self):
        self.api_key = get_config().get('api_key', '')
        self.timeout = 900
        self.api_endpoint = f"{baseurl}/v1/images/generations"
    ...
    def extract_image_urls(self, response_text):
        ...
        image_pattern = r'!\[.*?\]\((.*?)\)'
        matches = re.findall(image_pattern, response_text)
        matches = [m for m in matches if not str(m).startswith("data:image")]
        if not matches:
            url_pattern = r'https?://\S+\.(?:jpg|jpeg|png|gif|webp)'
            matches = re.findall(url_pattern, response_text)
        if not matches:
    ...
    def _add_base64_match(self, matches, seen, b64_data, mime_type="image/png"):
        cleaned = re.sub(r"\s+", "", str(b64_data or ""))
        if cleaned and cleaned not in seen:
            matches.append((cleaned, mime_type))
            seen.add(cleaned)

    def extract_base64_images(self, response_text):
        """Extract base64 image payloads from data URLs and b64_json fields."""
        if not response_text:
            return []
        matches = []
        seen = set()

        markdown_data_url_pattern = r'!\[[^\]]*\]\(data:image/(png|jpeg|jpg|webp);base64,([A-Za-z0-9+/=\r\n]+)\)'
        for mime_ext, b64_data in re.findall(markdown_data_url_pattern, response_text):
            mime_ext = "jpeg" if mime_ext == "jpg" else mime_ext
            self._add_base64_match(matches, seen, b64_data, f"image/{mime_ext}")

        quoted_data_url_pattern = r'["\']data:image/(png|jpeg|jpg|webp);base64,([^"\']+)["\']'
        for mime_ext, b64_data in re.findall(quoted_data_url_pattern, response_text):
            mime_ext = "jpeg" if mime_ext == "jpg" else mime_ext
            self._add_base64_match(matches, seen, b64_data, f"image/{mime_ext}")

        raw_data_url_pattern = r'data:image/(png|jpeg|jpg|webp);base64,([A-Za-z0-9+/=]+)'
        for mime_ext, b64_data in re.findall(raw_data_url_pattern, response_text):
            mime_ext = "jpeg" if mime_ext == "jpg" else mime_ext
            self._add_base64_match(matches, seen, b64_data, f"image/{mime_ext}")

        b64_json_pattern = r'"b64_json"\s*:\s*"([^"]+)"'
        for b64_data in re.findall(b64_json_pattern, response_text):
            try:
                cleaned = json.loads(f'"{b64_data}"')
            except Exception:
                cleaned = b64_data
            self._add_base64_match(matches, seen, cleaned, "image/png")

        return matches

    def download_image_to_tensor_url(self, url, timeout=30, output_format="png"):
        """Download an image URL, convert it to the selected format, and expose it via ComfyUI /view."""
        try:
            headers = {
            ...
            image = Image.open(BytesIO(response.content))
            image.load()
            local_url = _comfly_save_pil_image_url(
                image,
                filename_prefix="aiai/gpt_image_2",
                output_format=output_format,
            )
            return pil2tensor(image), local_url
        except Exception as e:
            print(f"[Comfly_gpt_image_2] Error downloading image from {url}: {str(e)}")
            return None, ""

    def _collect_image_fragments(self, value):
        fragments = []
        if isinstance(value, dict):
            for key, item in value.items():
                key_l = str(key).lower()
                if key_l == "b64_json" and item:
                    fragments.append(json.dumps({"b64_json": item}))
                    continue
                if key_l in ("url", "image_url") and isinstance(item, str):
                    fragments.append(item)
                    continue
                fragments.extend(self._collect_image_fragments(item))
        elif isinstance(value, list):
            for item in value:
                fragments.extend(self._collect_image_fragments(item))
        elif isinstance(value, str):
            if "data:image/" in value or value.startswith("http://") or value.startswith("https://"):
                fragments.append(value)
        return fragments

    def process(self, prompt, model="gpt-image-2", n=1, size="auto", format="png",
                quality="auto", max_retries=3, initial_timeout=900, api_key="",
                skip_error=False):
        ...
        try:
            ...
            pbar.update_absolute(20)
            print(f"[Comfly_gpt_image_2] Sending generation request to API with model: {model}")
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    response = requests.post(
                        ...
                        headers=self.get_headers(),
                        json=payload,
                        timeout=min(initial_timeout * (1.5 ** (attempt - 1)), self.timeout),
                    )
                    response.raise_for_status()
                    break
                    ...
                    time.sleep(min(2 ** (attempt - 1), 60))
            if response is None:
                raise RuntimeError("No response from image generation API")
            response.raise_for_status()
            result = response.json()
            ...
            for fragment in self._collect_image_fragments(result):
                if isinstance(fragment, str) and fragment.startswith(("http://", "https://")) and fragment not in image_urls:
                    image_urls.append(fragment)
                elif isinstance(fragment, str) and "data:image/" in fragment:
                    for item in self.extract_base64_images(fragment):
                        if item not in base64_images:
            ...
            if base64_images:
                for i, (b64_data, mime_type) in enumerate(base64_images):
                    print(f"[Comfly_gpt_image_2] Saving base64 image {i+1}/{len(base64_images)} as {format}")
                    pbar.update_absolute(min(80, 50 + (i+1) * 30 // len(base64_images)))
                    img_tensor, local_url = _comfly_base64_image_to_tensor_url(
                        b64_data,
                    ...
                    if local_url:
                        output_urls.append(local_url)

            if image_urls and not img_tensors:
                try:
                    for i, url in enumerate(image_urls):
                        ...
                        img_tensor, local_url = self.download_image_to_tensor_url(
                            url,
                            initial_timeout,
                            format,
                        )
                        if img_tensor is not None:
                            img_tensors.append(img_tensor)
                        output_urls.append(local_url or url)
                except Exception as e:
                    print(f"[Comfly_gpt_image_2] Error processing image URLs: {str(e)}")

            image_urls_string = "\n".join([u for u in output_urls if u])

            if img_tensors:
                combined_tensor = torch.cat(img_tensors, dim=0)
                pbar.update_absolute(100)
                return (combined_tensor, technical_response, image_urls_string)

            error_message = f"No image data decoded from response: {response_text[:1000]}"
            print(f"[Comfly_gpt_image_2] {error_message}")
            blank_img = Image.new('RGB', (1024, 1024), color='white')
            pbar.update_absolute(100)
            if not skip_error:
                raise RuntimeError(f"[Comfly_gpt_image_2] {error_message}")
            return (pil2tensor(blank_img), technical_response + f"\n**Error**: {error_message}", image_urls_string)
...
class Comfly_gpt_image_2_S2A:
    ...
    def generate_image(self, prompt, mode="text2img", model="gpt-image-2",
        ...
        try:
            ...
            if task_id.strip():
                print(f"[Comfly_gpt_image_2_S2A] Querying task status for task_id: {task_id}")
                return self._query_task_status(task_id, pbar)

            # 否则创建新的异步任务
            print(f"[Comfly_gpt_image_2_S2A] Creating new async task with mode: {mode}")
            ...
            if mode == "text2img":
                ...
                print(f"[Comfly_gpt_image_2_S2A] Submitting text2img async request: {payload}")
                response = requests.post(
                    f"{baseurl}/v1/images/generations",
                    headers=headers,
                    params=params,
                ...
                print(f"[Comfly_gpt_image_2_S2A] Processing {image_count} input images")
                ...
                print(f"[Comfly_gpt_image_2_S2A] Submitting img2img async request with {image_count} images")
                response = requests.post(
                    f"{baseurl}/v1/images/edits",
                    headers=headers,
                    params=params,
            ...
            if "task_id" in result:
                ...
                print(f"[Comfly_gpt_image_2_S2A] [ASYNC_RESPONSE] {json.dumps(result_info, ensure_ascii=False)}")
                ...
                while attempt < max_attempts:
                    ...
                    try:
                        ...
                        if query_response.status_code == 200:
                            ...
                            print(f"[Comfly_gpt_image_2_S2A] Task status (attempt {attempt}): {actual_status}")
                            ...
                            if actual_status in ("completed", "success", "done", "finished", "SUCCESS") or (actual_status == "unknown" and actual_data):
                                if actual_data:
                                    return self._process_image_data(actual_data, returned_task_id, model, mode, prompt, quality, size, seed, pbar)

                            elif actual_status in ("failed", "error", "FAILURE"):
                                error_msg = query_result.get("error", "Unknown error")
                                print(f"[Comfly_gpt_image_2_S2A] Task failed: {error_msg}")
                            ...
                            print(f"[Comfly_gpt_image_2_S2A] Query failed with status {query_response.status_code}")

                    except Exception as e:
                        print(f"[Comfly_gpt_image_2_S2A] Error querying task status: {str(e)}")

                # 超时未完成
                print(f"[Comfly_gpt_image_2_S2A] Task polling timed out")
                ...
                # 同步模式：直接返回图片数据
                print(f"[Comfly_gpt_image_2_S2A] Sync mode detected, processing {len(result['data'])} images directly")
    ...
    def _query_task_status(self, task_id, pbar):
        ...
        try:
            ...
            print(f"[Comfly_gpt_image_2_S2A] Querying task status: {query_url}")
            response = requests.get(
                query_url,
                headers=headers,
                timeout=self.timeout
            ...
            result = response.json()
            print(f"[Comfly_gpt_image_2_S2A] Task status response: {result}")
            ...
            error_message = f"Error querying task: {str(e)}"
            print(f"[Comfly_gpt_image_2_S2A] {error_message}")
            blank_image = Image.new('RGB', (1024, 1024), color='white')
            blank_tensor = pil2tensor(blank_image)
            pbar.update_absolute(100)
    ...
    def process(self, prompt, model, apikey="", seconds="15", size="1280x720", image=None, seed=0, private=True, skip_error=False):
        ...
        try:
            ...
            while attempts < max_attempts:
                ...
                try:
                    status_response = requests.get(
                        f"https://ai.kegeai.top/v1/videos/{task_id}",
                        headers=self.get_headers(),
                        timeout=self.timeout
                    )
...
class Comfly_sora2_new:
    ...
    def __init__(self):
        self.api_key = ""
        self.base_url = "https://ai.kegeai.top"
        self.timeout = 900
        self.session = requests.Session()
        self.model_mapping = {
            "sora-2": "sy_8",
...
class Comfly_Googel_Veo3:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "prompt": ("STRING", {"multiline": True}),
                "model": (["veo3", "veo3-fast", "veo3-pro", "veo3-fast-frames", "veo3-pro-frames", "veo3.1","veo3.1-fast","veo3.1-pro", "veo3.1-components", "veo3.1-4k", "veo3.1-pro-4k", "veo3.1-components-4k"], {"default": "veo3"}),
    ...
    def process(self, prompt, model, num_images=1, seed=0, image_way="image",
        ...
        try:
            ...
            if "queue.fal.run" in response_url:
                response_url = response_url.replace("https://queue.fal.run", "https://ai.kegeai.top")
            ...
            for i, img_data in enumerate(result_data["images"]):
                if "url" in img_data:
                    ...
                    if "queue.fal.run" in img_url:
                        img_url = img_url.replace("https://queue.fal.run", "https://ai.kegeai.top")
...
class Comfly_MiniMax_video:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "prompt": ("STRING", {"multiline": True}),
                "model": (["MiniMax-Hailuo-02", "T2V-01", "T2V-01-Director", "I2V-01-Director", "I2V-01-live", "I2V-01", "S2V-01","MiniMax-Hailuo-2.3-Fast","MiniMax-Hailuo-2.3"], {"default": "MiniMax-Hailuo-02"}),
...
class OpenAISoraAPIPlus:
    """
    ComfyUI自定义节点：ai.kegeai.top Sora-2 视频生成（OpenAI兼容流式接口）
    - 参考 openai_chat_api_node.py 的结构与风格
    - 通过 ai.kegeai.top 的 /chat/completions 接口，以 stream=True 获取流式增量内容
    - 适配示例返回：每行均为 JSON，字段为 choices[0].delta.content
    - 超时时间：600 秒（10 分钟）
    输入参数：
      - base_url: 默认 https://ai.kegeai.top/v1
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "base_url": ("STRING", {"default": "https://ai.kegeai.top/v1", "multiline": False}),
    ...
    def generate(self, base_url, model, api_key, user_prompt,image=None,hd=True,duration=15,aspect_ratio="16:9", skip_error=False):
        """
        调用 ai.kegeai.top 的 sora-2 模型进行视频生成（流式）。
        请求：
...
class Comfly_LLm_API:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "api_baseurl": ("STRING", {"multiline": True, "default": "https://ai.kegeai.top/v1"}),
                "api_key": ("STRING", {"default": ""}),
    ...
    FUNCTION = "aiai_run_llmapi"
    CATEGORY = "aiai/LLM"

    def aiai_run_llmapi(self, api_baseurl, api_key, model, role, prompt, temperature, seed, ref_image=None, video=None, skip_error=False):
        if OpenAI is None:
            if not skip_error:
...
class Comfly_wan2_6_API:
    ...
    def convert_image_to_base64(self, image_tensor):
        ...
        try:
            ...
            print(f"[aiai_WanVideo INFO] Original image size: {original_size[0]}x{original_size[1]}")
            ...
            if max(original_size) > max_dimension:
                ...
                print(f"[aiai_WanVideo INFO] Resized image to: {new_size[0]}x{new_size[1]}")
            ...
            for format_name, mime_type, save_kwargs in formats_to_try:
                try:
                    ...
                    if len(image_bytes) < smallest_size:
                        ...
                        print(f"[aiai_WanVideo INFO] {format_name} format: {base64_size_mb:.2f}MB base64")
                    ...
                    print(f"[aiai_WanVideo WARNING] Failed to save as {format_name}: {format_error}")
                    continue
            
            if best_result:
                final_size_mb = len(best_result.split(',')[1]) / (1024 * 1024)
                print(f"[aiai_WanVideo INFO] Final base64 size: {final_size_mb:.2f}MB")
                
                if final_size_mb > 3.0:
                    print(f"[aiai_WanVideo WARNING] Base64 data is large ({final_size_mb:.2f}MB), may cause API issues")
            ...
            print(f"[aiai_WanVideo ERROR] Image base64 conversion error: {str(e)}")
            return None
    ...
    def generate_video(self, prompt, api_key, resolution, duration, image=None, audio_url="", prompt_extend=True, shot_type="multi", audio_enabled=True, skip_error=False):
        ...
        try:
            ...
            if image is not None:
                ...
                print(f"[aiai_Wan26_I2V INFO] Image converted to base64 successfully")
            ...
            print(f"[aiai_WanVideo INFO] Request body: {json.dumps(log_body, indent=2, ensure_ascii=False)}")
            print(f"[aiai_WanVideo INFO] Sending request to: {url}")
            ...
            if response.status_code == 200:
                ...
                if task_id:
                    print(f"[aiai_WanVideo INFO] Task created. Task ID: {task_id}")
                    
                    # Poll for result
                    video_url = self.poll_task_status(task_id)
                    
                    if video_url:
                        print(f"[aiai_WanVideo INFO] Downloading video from: {video_url}")
                        video_adapter = ComflyVideoAdapter(video_url)
                    ...
                    print(f"[aiai_WanVideo ERROR] No task ID: {result}")
                    if not skip_error:
                        raise RuntimeError(f"[Comfly_wan2_6_API] {error_msg}")
                ...
                print(f"[aiai_WanVideo ERROR] API error ({status_code}): {error_message}")
                if not skip_error:
                    raise RuntimeError(f"[Comfly_wan2_6_API] {error_message}")
                return (EmptyVideoAdapter(), f"API error: {status_code} - {error_message}", "")
                
        except Exception as e:
            print(f"[aiai_WanVideo ERROR] Video generation error: {str(e)}")
            if not skip_error:
                raise
    ...
    def poll_task_status(self, task_id):
        ...
        while time.time() - start_time < max_poll_time:
            try:
                print(f"[aiai_WanVideo INFO] Querying task: {query_url}")
                ...
                if response.status_code == 200:
                    ...
                    if task_status == 'SUCCEEDED':
                        ...
                        if video_url:
                            print(f"[aiai_WanVideo INFO] Video ready: {video_url}")
                            return video_url
                        else:
                            print(f"[aiai_WanVideo ERROR] No video URL: {result}")
                            return None
                    elif task_status == 'FAILED':
                        error_msg = result.get('output', {}).get('message', 'Unknown error')
                        print(f"[aiai_WanVideo ERROR] Task failed: {error_msg}")
                        return None
                    elif task_status in ['PENDING', 'RUNNING']:
                        elapsed = time.time() - start_time
                        remaining = max_poll_time - elapsed
                        print(f"[aiai_WanVideo INFO] Status: {task_status} ({elapsed:.1f}s elapsed, {remaining:.1f}s remaining)")
                        time.sleep(poll_interval)
                        continue
                    else:
                        print(f"[aiai_WanVideo WARNING] Unknown status: {task_status}")
                        time.sleep(poll_interval)
                        continue
                else:
                    print(f"[aiai_WanVideo ERROR] Query failed: {response.status_code}")
                    return None
                    
            except Exception as e:
                print(f"[aiai_WanVideo ERROR] Query error: {str(e)}")
                return None
        
        print("[aiai_WanVideo ERROR] Polling timeout")
        return None
...
NODE_CLASS_MAPPINGS = {
    ...
    "Comfly_gpt_image_2_S2A": Comfly_gpt_image_2_S2A,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "Comfly_api_set": "aiai API Settings",
    "OpenAI_Sora_API_Plus": "OpenAI Sora API Plus节点",
    "OpenAI_Sora_API": "OpenAI Sora API节点",
    "Comfly_Mj": "aiai_Mj", 
    "Comfly_mjstyle": "aiai_mjstyle",
    "Comfly_upload": "aiai_upload",
    "Comfly_Mju": "aiai_Mju",
    "Comfly_Mjv": "aiai_Mjv",  
    "Comfly_Mj_swap_face": "aiai MJ Face Swap",
    "Comfly_kling_text2video": "aiai_kling_text2video",
    "Comfly_kling_image2video": "aiai_kling_image2video",
    "Comfly_kling_multi_image2video": "aiai_kling_multi_image2video",
    "Comfly_video_extend": "aiai_video_extend",
    "Comfly_lip_sync": "aiai_lip_sync",
    "ComflyGeminiAPI": "aiai Gemini API",
    "ComflySeededit": "aiai Doubao SeedEdit2.0",
    "ComflyChatGPTApi": "aiai ChatGPT Api",
    "Comfly_sora2_openai": "aiai_sora2_openai", 
    "Comfly_sora2": "aiai_sora2", 
    "Comfly_sora2_chat": "aiai_sora2_chat", 
    "Comfly_sora2_character": "aiai Sora2 Character",  
    "Comfly_sora2_new": "aiai Sora2 New",
    "ComflyJimengApi": "aiai Jimeng API", 
    "Comfly_gpt_image_1_edit": "aiai_gpt_image_1_edit",
    "Comfly_gpt_image_1": "aiai_gpt_image_1",
    "Comfly_gpt_image_2": "aiai_gpt_image_2",
    "Comfly_gpt_image_2_S2A": "aiai GPT Image 2 S2A",
    "ComflyJimengVideoApi": "aiai Jimeng Video API",
    "Comfly_Flux_Kontext": "aiai_Flux_Kontext",
    "Comfly_Flux_Kontext_Edit": "aiai_Flux_Kontext_Edit",
    "Comfly_Flux_Kontext_bfl": "aiai_Flux_Kontext_bfl",
    "Comfly_Flux_2_Pro": "aiai_Flux_2_Pro",
    "Comfly_Flux_2_Flex": "aiai_Flux_2_Flex",
    "Comfly_Flux_2_Max": "aiai_Flux_2_Max",
    "Comfly_Googel_Veo3": "aiai Google Veo3",
    "Comfly_Googel_Veo3_Lite": "aiai Google Veo3 Lite",
    "ComflyGeminiTextOnly": "aiai GeminiTextOnly",
    "Comfly_mj_video": "aiai MJ Video",
    "Comfly_mj_video_extend": "aiai MJ Video Extend",
    "Comfly_qwen_image": "aiai_qwen_image",
    "Comfly_qwen_image_edit": "aiai_qwen_image_edit",
    "Comfly_Doubao_Seedream": "aiai Doubao Seedream3.0",
    "Comfly_Doubao_Seedream_4": "aiai Doubao Seedream4.0",
    "Comfly_Doubao_Seedream_4_5": "aiai Doubao Seedream4.5",
    "Comfly_Doubao_Seededit": "aiai Doubao Seededit3.0",
    "Comfly_MiniMax_video": "aiai MiniMax Hailuo Video",
    "Comfly_suno_description": "aiai Suno Description",
    "Comfly_suno_lyrics": "aiai Suno Lyrics",
    "Comfly_suno_custom": "aiai Suno Custom",
    "Comfly_suno_upload": "aiai Suno Upload",
    "Comfly_suno_upload_extend": "aiai Suno Upload Extend", 
    "Comfly_suno_cover": "aiai Suno Cover",
    "Comfly_vidu_img2video": "aiai Vidu Image2Video",
    "Comfly_vidu_text2video": "aiai_vidu_text2video",
    "Comfly_vidu_ref2video": "aiai_vidu_ref2video",
    "Comfly_vidu_start-end2video": "aiai_vidu_start-end2video",
    "Comfly_nano_banana": "aiai_nano_banana",
    "Comfly_nano_banana_fal": "aiai_nano_banana_fal",
    "Comfly_nano_banana_edit": "aiai_nano_banana_edit",
    "Comfly_nano_banana2_edit": "aiai_nano_banana2_edit",
    "Comfly_nano_banana2_edit_S2A": "aiai_nano_banana2_edit_S2A",
    "Comfly_gemini_3_1_flash_image_edit_S2A": "aiai Gemini 3.1 Flash Image Edit S2A",
    "ComflyGrok3VideoApi": "aiai Grok3 Video",
    "ComflyGrok3VideoApi30S": "aiai Grok3 Video 30S",
    "Comfly_Z_image_turbo": "aiai_Z_Image_Turbo",
    "Comfly_LLm_API": "aiai LLM API",
    "Comfly_Doubao_Seedance2_0": "aiai Doubao Seedance 2.0",
    "Comfly_Doubao_Seedance2_0_Asset": "aiai Doubao Seedance 2.0 Asset Upload",
    "Comfly_Doubao_Seedance2_0_AssetIdBundle": "aiai Doubao_Seedance2_0_AssetIdBundle",
    "Comfly_gpt_image_2_official":"aiai-gpt-image-2-official",
    "Comfly_gpt_image_2_official_ratio":"aiai-gpt-image-2-official_ratio",
    "Comfly_wan2_6_API": "aiai WanX 2.6 Video"
}
